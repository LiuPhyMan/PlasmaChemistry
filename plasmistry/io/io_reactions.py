...
def read_reactionFile(file_path, start_line=-math.inf, end_line=math.inf):
    ...
    # ------------------------------------------------------------------------------------------- #
    rcntM, prdtM, dHM, k_strM = pd.Series(), pd.Series(), pd.Series(), \
                                pd.Series()
    pre_exec_list = []
    envir_vars = dict()
    ...
    while i_line < len(rctn_list):
        ...
        #             /home/liujinbao/Documents/CODE/PlasmaChemistry
        #             /CrossSectionFile/H2
        #           }
        #       %H2_vib% = \
        #           {   H2      H2(v1)  H2(v2)  H2(v3)  H2(v4)  H2(v5) H2(v6)
        #           H2(v7) H2(v8) H2(v9)
