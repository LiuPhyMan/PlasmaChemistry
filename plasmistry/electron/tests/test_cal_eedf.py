...
class test_CAL_EEDF(TestCase):
    ...
    def test_set_flux_electric_field(self):
        ...
        eedf._set_flux_electric_field(total_species_density=np.array([1, 2, 3, 4, 5]))

        def dndt(t, y, _eedf):
            _eedf.density_in_J = y
            _eedf._set_flux_electric_field(total_species_density=np.array([1, 2, 3, 4, 5]))
            return _eedf._get_deriv_ef()
    ...
    def test_set_flux_elastic_colli(self):
        ...
        eedf._set_flux_elastic_colli(total_species_density=np.array([1, 2, 3, 4, 5]))

        # species conservation
        def dndt(t, y, _eedf):
            _eedf.density_in_J = y
            _eedf._set_flux_elastic_colli(total_species_density=np.array([1, 2, 3, 4, 5]))
            return _eedf._get_deriv_el()
        ...
        sol = ode_ivp(deriv_func=dndt,
                      ...
                      show_time=False)
        eedf.density_in_J = sol.y[-1]
        assert_allclose(eedf.electron_density, 1.0, rtol=1e-2)
    ...
    def test_set_flux_ee_colli(self):
        ...
        assert_allclose(density_0, eedf.electron_density, rtol=1e-2)
        assert_allclose(Te_0, eedf.electron_temperature, rtol=1e-2)
    ...
    def test_set_rate_const_matrix(self):
        ...
        def dndt(t, y, _eedf, density):
            _eedf.set_density_in_J(y)
            return _eedf._get_electron_rate_e_inelas(density=density)

        y_0 = 1e16 * get_maxwell_eedf(eedf.energy_point, Te_eV=1.0)
        density = 1e25 * np.array([1, 2, 3, 4, 5, 6])
        # print(eedf._get_molecule_rate_e_inelas(density=density) * 0.291 * const.eV2J)
        # print((eedf._get_electron_rate_e_inelas(
...
r"""
    def test_electric_field_elastic_colli_flux(self):
        eedf = self.case_0
        eedf._set_crostn_elastic(bg_molecule_elas=['CO', 'N2', 'CO2'])
        # %%------------------------------------------------------------------------------------- #
        ...
        eedf._set_flux_electric_field()
        eedf._set_electric_field_flux(electric_field=10.0,
                                     density_total=10.0,
